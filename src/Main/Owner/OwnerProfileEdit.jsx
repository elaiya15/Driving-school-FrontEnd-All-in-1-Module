...
const schema = yup.object().shape({
  ...
  AlternativeNumber: yup
    .string()
    .nullable()
    .matches(/^[0-9]{10}$/, "Alternative Number must be 10 digits.")
    .optional(),
...
const OwnerProfileEdit = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(`${URL}/api/v1/organization/${id}`, formData, branchHeaders());
  ...
  return (
    <div className="p-4">
      <h2 className="mb-4 text-2xl font-semibold">Update Owner</h2>
      <form onSubmit={handleSubmit(onSubmit)}>
        {/* Basic Info */}
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 gap-y-10">
          {["ownerName", "email", "mobileNumber", "AlternativeNumber"].map(
            (field) => (
        ...
        <div className="grid grid-cols-1 gap-4 mt-8 md:grid-cols-2 gap-y-10">
          {["username", "password"].map((field) => (
            <div
              className="relative"
              key={field}
              style={{ display: field === "password" ? "none" : "block" }}
            >
              <input
                type="text"
                ...
                {...register(field)}
                placeholder=" "
                className={`peer w-full px-2.5 pb-2.5 pt-4 text-sm text-gray-900 bg-transparent border border-gray-300 rounded-md focus:outline-none focus:ring-0 focus:border-blue-500 ${field === "username" ? "cursor-not-allowed" : ""}`}
              />
              <label
                htmlFor={field}
                className="absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-placeholder-shown:top-1/2 peer-placeholder-shown:scale-100 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1"

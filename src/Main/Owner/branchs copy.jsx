...
import { URL } from "../../App";
import {setBranchCookie,removeBranchCookie} from './../../Components/utils/BranchCookie.jsx';
const BranchTable = () => {
    ...
    const fetchData = async () => {
      ...
      try {
       ...
       if (!user || !user.organizationId) {
        ...
        const res = await axios.get(`${URL}/api/branches/organization_branches/${user.organizationId}`, {
          params,
...
const handleAdminModeToggle = (branchId, enabled) => {
  setBranchCookie(branchId);
  // API call to enable/disable admin mode for this user
};
  ...
  return (
    <div className="p-4">
      <div className="flex flex-col gap-4 mb-4 md:flex-row md:items-center md:justify-between">
        <h3 className="text-xl font-bold text-center md:text-left">Branch List</h3>
        <button
            ...
            <table className="w-full text-sm text-left text-gray-500">
              <thead className="text-sm text-gray-700 bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4">Switch Branch Admin</th>
                </tr>
              </thead>
              <tbody>
                {branches.length > 0 ? (
  branches.map((branch, index) => (
    <tr key={branch._id} className="bg-white border-b">
  ...
  <td className="px-6 py-4">
  <i className="text-blue-600 fa-solid fa-toggle-on"></i>
</td>

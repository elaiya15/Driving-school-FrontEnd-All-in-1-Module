...
const InsEdit = () => {
  ...
  const fetchInstructor = async () => {
    try {
      const { data } = await axios.get(`${BURL}/api/v2/instructor/${id}`, branchHeaders());
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(`${BURL}/api/v2/instructor/${id}`, formData,branchHeaders());
  ...
  if (loading) return <p className="py-4 text-center text-blue-600">Loading...</p>;

  return (
    <div className="p-4">
      <h2 className="mb-4 text-2xl font-semibold">Update Instructor</h2>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mt-6">
          ...
          <label htmlFor="photo" className="relative flex items-center justify-center w-full overflow-hidden border-2 border-gray-300 border-dashed rounded-lg cursor-pointer h-36">
            {preview ? (
  ...
  {["username", "password"].map((id) => (
    <div
      className="relative"
      key={id}
      // 👇 Completely hide the password field for now
      style={{ display: id === "password" ? "none" : "block" }}
    >
      <input
        type={id === "password" ? "password" : "text"}
        {...register(id)}
        ...
        className="peer w-full px-2.5 pb-2.5 pt-4 text-sm text-gray-900 bg-gray-100 border border-gray-300 rounded-md focus:outline-none focus:ring-0 focus:border-blue-500 disabled:cursor-not-allowed"
      />
      <label
        htmlFor={id}
      ...
      {errors[id] && (
        ...
        <div className="flex flex-col mt-6 space-y-4 md:flex-row md:justify-end md:space-y-0 md:space-x-4">
          <button type="button" onClick={() => navigate(-1)} className="px-4 py-2 text-white bg-gray-600 rounded-md hover:bg-gray-800">Back</button>
          <button type="submit" disabled={submitLoading} className="px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-800">
